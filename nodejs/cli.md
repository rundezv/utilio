# CLI (Command Line Interface)

- [goenning/**google-indexing-script**](https://github.com/goenning/google-indexing-script) - Script to get your site indexed on Google in less than 48 hours. **`npm-scripts`** **`seo`**  
  ![](https://img.shields.io/github/stars/goenning/google-indexing-script?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/goenning/google-indexing-script?style=social&label=Update)
- [google/**zx**](https://github.com/google/zx) - A tool for writing better scripts. **`api`** **`shell`** **`promise`** **`cross-platform`**  
  ![](https://img.shields.io/github/stars/google/zx?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/google/zx?style=social&label=Update)
- [dsherret/**dax**](https://github.com/dsherret/dax) - Cross-platform shell tools for Deno and Node.js inspired by zx. **`api`** **`shell`** **`promise`** **`cross-platform`**  
  ![](https://img.shields.io/github/stars/dsherret/dax?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/dsherret/dax?style=social&label=Update)
- [bcomnes/**npm-run-all2**](https://github.com/bcomnes/npm-run-all2) - A CLI tool to run multiple npm-scripts in parallel or sequential. (Maintenance fork) **`npm-scripts`** **`api`** **`cross-platform`**  
  ![](https://img.shields.io/github/stars/bcomnes/npm-run-all2?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/bcomnes/npm-run-all2?style=social&label=Update)
- [yargs/**yargs**](https://github.com/yargs/yargs) - yargs the modern, pirate-themed successor to optimist. **`api`** **`parser`**  
  ![](https://img.shields.io/github/stars/yargs/yargs?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/yargs/yargs?style=social&label=Update)
- [SBoudrias/**Inquirer.js**](https://github.com/SBoudrias/Inquirer.js) - A collection of common interactive command line user interfaces. **`api`** **`parser`** **`prompt`**  
  ![](https://img.shields.io/github/stars/SBoudrias/Inquirer.js?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/SBoudrias/Inquirer.js?style=social&label=Update)
- [bombshell-dev/**clack**](https://github.com/bombshell-dev/clack) - Effortlessly build beautiful command-line apps. **`api`** **`parser`** **`prompt`**  
  ![](https://img.shields.io/github/stars/bombshell-dev/clack?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/bombshell-dev/clack?style=social&label=Update)
- [antonmedv/**srf**](https://github.com/antonmedv/srf) - A file server. **`server`**  
  ![](https://img.shields.io/github/stars/antonmedv/srf?style=social&label=Star)
  ![](https://img.shields.io/github/last-commit/antonmedv/srf?style=social&label=Update)
